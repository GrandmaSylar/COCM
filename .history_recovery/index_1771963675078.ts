  ...
  try {
    ...
    return c.json({
      attendanceStats: {
        ...
        percentage: 0 // Cannot easily calculate max services without overall church schedule
      },
      recentActivity: recentActivity
    });
  } catch (e) { return c.json({ error: 'Internal server error' }, 500); }
...
app.get('/children/members/:id/attendance-history', async (c) => {
  try {
    ...
    if (childError || !child) return c.json({ error: 'Child not found' }, 404);
    ...
    if (recordsErr) return c.json({ error: 'Failed to fetch records' }, 500);
    ...
    if (entriesErr) return c.json({ error: 'Failed to fetch entries' }, 500);
...
// Helper: check if user can write expenses (admin/dev or has 'expenses' tab access)
async function canWriteExpenses(userId: string): Promise<boolean> {
  const { data: prof } = await supabase.from('profiles').select('role').eq('id', userId).single();
  if (prof?.role === 'admin' || prof?.role === 'dev') return true;
...
// Helper: generate next reference number EXP-YYYY-NNN
async function generateExpenseRefNo(): Promise<string> {
  ...
  let seq = 1;
  if (data && data.length > 0) {
    const last = data[0].reference_no as string;
...
app.get('/expenses', async (c) => {
  try {
    ...
    let query = supabase
      ...
      .select(`
        *,
        requested_by_member:members!requested_by(id, first_name, last_name),
        recommended_by_member:members!recommended_by(id, first_name, last_name),
        approved_by_member:members!approved_by(id, first_name, last_name),
        created_by_profile:profiles!created_by(id, name)
      `)
      .order('created_at', { ascending: false });
    ...
    const mapped = (data || []).map((r: any) => ({
      ...toCamelCase(r),
      requestedByName: r.requested_by_member ? `${r.requested_by_member.first_name || ''} ${r.requested_by_member.last_name || ''}`.trim() : null,
      recommendedByName: r.recommended_by_member ? `${r.recommended_by_member.first_name || ''} ${r.recommended_by_member.last_name || ''}`.trim() : null,
      approvedByName: r.approved_by_member ? `${r.approved_by_member.first_name || ''} ${r.approved_by_member.last_name || ''}`.trim() : null,
      createdByName: r.created_by_profile?.name || null,
    }));
...
app.get('/expenses/:id', async (c) => {
  try {
    ...
    if (error || !data) return c.json({ error: 'Expense not found' }, 404);

    return c.json({
      ...toCamelCase(data),
      requestedByName: data.requested_by_member ? `${data.requested_by_member.first_name || ''} ${data.requested_by_member.last_name || ''}`.trim() : null,
      recommendedByName: data.recommended_by_member ? `${data.recommended_by_member.first_name || ''} ${data.recommended_by_member.last_name || ''}`.trim() : null,
      approvedByName: data.approved_by_member ? `${data.approved_by_member.first_name || ''} ${data.approved_by_member.last_name || ''}`.trim() : null,
      createdByName: data.created_by_profile?.name || null,
    });
  } catch (error) {
...
app.post('/expenses', async (c) => {
  try {
    const user = await getUserFromToken(c.req.raw);
    if (!user) return c.json({ error: 'Unauthorized' }, 401);
    if (!await canWriteExpenses(user.id)) return c.json({ error: 'Forbidden' }, 403);

    const body = await c.req.json();

    if (!body.serviceDate || !body.serviceType || !body.details || !body.amount || !body.paymentMethod) {
      return c.json({ error: 'Missing required fields: serviceDate, serviceType, details, amount, paymentMethod' }, 400);
    }

    const referenceNo = await generateExpenseRefNo();

    console.log("DEBUG /expenses create:", {
      referenceNo,
    ...
    if (body.requestedBy) {
      const { data: m1 } = await supabase.from('members').select('id').eq('id', body.requestedBy).maybeSingle();
      if (!m1) console.error("DEBUG: requestedBy member ID does not exist in DB:", body.requestedBy);
    }
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        reference_number: body.paymentMethod === 'cash' ? null : (body.referenceNumber || null),
        status:           'approved',
        requested_by:     body.requestedBy || null,
        recommended_by:   body.recommendedBy || null,
        approved_by:      body.approvedBy || null,
        created_by:       user.id,
      })
      .select(`
        *,
        requested_by_profile:profiles!expense_requisitions_requested_by_fkey(id, name),
        recommended_by_profile:profiles!expense_requisitions_recommended_by_fkey(id, name),
        approved_by_profile:profiles!expense_requisitions_approved_by_fkey(id, name),
        created_by_profile:profiles!expense_requisitions_created_by_fkey(id, name)
      `)
      .single();
    ...
    await logActivity({
      ...
      action: 'create_expense', entityType: 'expense', entityId: data.id,
      description: `Created expense requisition ${referenceNo} for GHS ${body.amount}`,
    });

    return c.json({
      ...toCamelCase(data),
      requestedByName: data.requested_by_profile?.name || null,
      recommendedByName: data.recommended_by_profile?.name || null,
      approvedByName: data.approved_by_profile?.name || null,
      createdByName: data.created_by_profile?.name || null,
    }, 201);
  } catch (error) {
...
app.put('/expenses/:id', async (c) => {
  try {
    ...
    await logActivity({
      userId: user.id, userName: prof.name, userRole: prof.role,
      action: 'update_expense', entityType: 'expense', entityId: id,
      description: `Updated expense requisition ${data.reference_no}`,
    });
...
app.delete('/expenses/:id', async (c) => {
  try {
    ...
    if (!user) return c.json({ error: 'Unauthorized' }, 401);
    const { data: prof } = await supabase.from('profiles').select('role').eq('id', user.id).single();
    if (prof?.role !== 'admin' && prof?.role !== 'dev') return c.json({ error: 'Forbidden: Only admin/dev can delete expenses' }, 403);

    const id = c.req.param('id');
    const { error } = await supabase.from('expense_requisitions').delete().eq('id', id);
    if (error) return c.json({ error: error.message }, 500);
