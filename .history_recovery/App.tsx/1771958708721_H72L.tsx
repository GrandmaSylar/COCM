...
import { SystemOptionsProvider } from './components/SystemOptionsContext';
...
const AddExpense = lazy(() => import('./components/Expenses').then(module => ({ default: module.AddExpense })));
const ExpenseDetail = lazy(() => import('./components/Expenses').then(module => ({ default: module.ExpenseDetail })));
...
type AppPage = 'login' | 'signup' | 'forgot-password' | 'otp-verification' | 'dashboard' | 'members' | 'add-member' | 'edit-member' | 'member-profile' |
               ...
               'expenses' | 'add-expense' | 'expense-detail';
...
function AppContent() {
  ...
  const handleAddExpense = () => navigateTo('add-expense');

  const handleViewExpense = (id: string) => {
    setSelectedExpenseId(id);
    navigateTo('expense-detail');
  };
