...
export function EditExpense({ expenseId, onBack, onSaved }: EditExpenseProps) {
  const { user } = useAuth();
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  disabled={!canEditFormId}
                  className={formIdError ? 'border-red-500 focus-visible:ring-red-500' : (!canEditFormId ? 'bg-muted' : '')}
                  required
                />
              ...
              <div className="space-y-2">
                ...
                <Select 
                  ...
                  onValueChange={(val) => {
                    const method = [...activeMethods, ...(pmData || [])].find((m: any) => m.id === val);
                    setFormData(prev => ({ 
                      ...prev, 
                      paymentMethodId: val, 
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    {formData.paymentMethodId && !activeMethods.some((m:any) => m.id === formData.paymentMethodId) && (
                      <SelectItem value={formData.paymentMethodId}>{formData.paymentMethodName} (Inactive)</SelectItem>
                    )}
                  </SelectContent>
                </Select>
              </div>
